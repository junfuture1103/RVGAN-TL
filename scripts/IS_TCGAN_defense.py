...
# PGD 적대적 샘플 생성
def pgd_attack(model, X, y, epsilon=0.2, alpha=0.02, num_iter=1000):
    X_adv = X.copy()
...
new_X_train = pd.concat([X_train, misclassified_samples])
new_y_train = np.concatenate([y_train, misclassified_labels])
